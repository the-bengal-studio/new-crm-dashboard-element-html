(function () {
  let r = document.querySelector(".check-all");
  ...
  document.querySelector(".invoice-add-item").onclick = () => {
    ...
    (document.querySelector(".invoice-body").innerHTML += e), (i = i + 1);
  };
  var c = document.querySelectorAll(".quantity-minus"),
    a = document.querySelectorAll(".quantity-plus"),
    u = 0,
    s = 30;
  c.forEach((e) => {
    e.onclick = function (n) {
      let t = n.currentTarget.nextElementSibling.value;
      t > u && ((t = t - 1), (n.currentTarget.nextElementSibling.value = t));
    };
  }),
    a.forEach((e) => {
      e.onclick = function (n) {
        let t = n.currentTarget.previousElementSibling.value;
        t < s &&
          ((t = Number(t) + 1),
          (n.currentTarget.previousElementSibling.value = t));
      };
    }),
    ...
    document.querySelectorAll(".invoice-btn").forEach((e) => {
      e.onclick = () => {
        e.closest(".invoice-list").remove();
      };
    });
})();
window.changeTheInfo = (r, l, i, c, a, u, s) => {
  (document.querySelector(".invoice-title").innerHTML = r),
    document.querySelectorAll(".company-name").forEach((o) => {
      o.value = l;
    }),
    document.querySelectorAll(".company-address").forEach((o) => {
      o.value = i;
    }),
    document.querySelectorAll(".company-address1").forEach((o) => {
      o.value = c;
    }),
    (document.querySelector(".invoice-id").value = a),
    (document.querySelector(".create-date").value = u),
    (document.querySelector(".due-date").value = s);
};
